...
const isFormValid = computed(() => {
    ...
    return form.value.phone.trim() !== '' && hasIdentity;
});
...
const submit = () => {
    ...
    if (confirm("Pastikan data sudah benar, pengajuan tidak bisa diubah setelah dikirim")) {
        ...
        if (form.value.domicile_document_url) {
            formData.append('domicile_document', form.value.domicile_document_url);
        }
        if (form.value.project_ownership_document_url) {
            formData.append('project_ownership_document', form.value.project_ownership_document_url);
        }
...
<template>
    ...
    <ProfileLayout>
        <div class="space-y-6 max-w-4xl">
            <div>
                <h2 class="text-2xl font-bold tracking-tight text-foreground">Verifikasi Identitas</h2>
                <p class="text-muted-foreground mt-2">
                    Verifikasi identitas Anda untuk mendapatkan kepercayaan dari arsitek dan fitur penuh sebagai Client.
                </p>
            </div>
            ...
            <Alert v-if="isVerified" class="bg-emerald-50 text-emerald-600 border-emerald-200">
                ...
                <AlertDescription>
                    Identitas Anda telah berhasil diverifikasi oleh admin. Data di bawah ini sudah tidak dapat diubah.
                </AlertDescription>
            </Alert>
            ...
            <Alert v-else-if="verificationStatus === 'rejected'" class="bg-rose-50 text-rose-600 border-rose-200">
                ...
                <AlertTitle>Pengajuan Identitas Ditolak</AlertTitle>
                <AlertDescription>
                    <p>Pengajuan verifikasi identitas Anda ditolak oleh admin.</p>
            ...
            <Card>
                ...
                <CardContent class="space-y-6">
                    ...
                    <!-- Domisili -->
                    <div class="space-y-3">
                        <Label>Surat Keterangan Domisili <span class="text-muted-foreground font-normal">(Opsional)</span></Label>
                        <div class="border-2 border-dashed rounded-xl p-6 text-center hover:bg-muted/50 transition-colors" :class="{'opacity-50 pointer-events-none': isVerified || verificationStatus === 'pending'}">
                            <input 
                                type="file" 
                                id="domisili" 
                                class="hidden" 
                                accept=".pdf,.jpg,.jpeg,.png"
                                @change="(e) => handleFileUpload(e, 'domicile_document_url')"
                                :disabled="isVerified || verificationStatus === 'pending'"
                            >
                            <label for="domisili" class="cursor-pointer flex flex-col items-center gap-3">
                                <div class="w-12 h-12 bg-primary/10 text-primary rounded-full flex items-center justify-center">
                                    <Upload class="w-6 h-6" />
                                </div>
                                <div class="space-y-1">
                                    <p class="text-sm font-medium text-foreground">Klik untuk upload Surat Domisili</p>
                                    <p class="text-xs text-muted-foreground">PDF, JPG, PNG (Max. 5MB)</p>
                                </div>
                            </label>
                            <p v-if="fileNames.domicile_document_url" class="mt-4 text-sm font-bold text-primary">
                    ...
                    <!-- Bukti Lahan -->
                    <div class="space-y-3">
                        <Label>Bukti Kepemilikan Proyek / Lahan <span class="text-muted-foreground font-normal">(Opsional)</span></Label>
                        <div class="border-2 border-dashed rounded-xl p-6 text-center hover:bg-muted/50 transition-colors" :class="{'opacity-50 pointer-events-none': isVerified || verificationStatus === 'pending'}">
                            <input 
                                type="file" 
                                id="lahan" 
                                class="hidden" 
                                accept=".pdf,.jpg,.jpeg,.png"
                                @change="(e) => handleFileUpload(e, 'project_ownership_document_url')"
                                :disabled="isVerified || verificationStatus === 'pending'"
                            >
                            <label for="lahan" class="cursor-pointer flex flex-col items-center gap-3">
                                <div class="w-12 h-12 bg-primary/10 text-primary rounded-full flex items-center justify-center">
                                    <Upload class="w-6 h-6" />
                                </div>
                                <div class="space-y-1">
                                    <p class="text-sm font-medium text-foreground">Klik untuk upload Bukti Lahan</p>
                                    <p class="text-xs text-muted-foreground">PDF, JPG, PNG (Max. 5MB)</p>
                                </div>
                            </label>
                            <p v-if="fileNames.project_ownership_document_url" class="mt-4 text-sm font-bold text-primary">
