...
const form = useForm({
  company_name: props.companyProfile?.company_name || props.user.name || '',
  industry: props.companyProfile?.industry || '',
  company_size: props.companyProfile?.company_size || '1-10',
  location: props.companyProfile?.location || '',
...
const userInitials = computed(() => {
  return (props.companyName || props.user.name).split(' ').map(n => n[0]).join('').toUpperCase().substring(0, 2);
});
</script>
    ...
    <div class="space-y-6">
      <Card class="border-border/60 shadow-sm overflow-hidden rounded-2xl bg-white">
        ...
        <div class="h-48 w-full bg-[#f8fafc] relative flex items-center justify-center border-b border-border/40">
           ...
           <div class="absolute right-12 bottom-0 h-40 w-64 opacity-50 select-none pointer-events-none hidden md:block text-right">
              <img src="https://illustrations.popsy.co/slate/creative-work.svg" class="h-full w-full object-contain object-bottom" alt="Illustration" />
           </div>
        </div>
           ...
           <div class="px-8 pb-10">
              ...
              <div class="relative -mt-12 mb-8 flex justify-center lg:justify-start">
                 <div class="relative group">
                    <Avatar class="h-28 w-28 rounded-2xl border-4 border-white shadow-md">
              ...
              <form @submit.prevent="submit" class="space-y-10">
                 <div class="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-8">
                    <div class="space-y-2 md:col-span-2">
                       <Label for="company_name" class="text-[13px] font-bold text-slate-700">Company Name<span class="text-destructive ml-0.5">*</span></Label>
                       <Input 
                          id="company_name" 
                          v-model="form.company_name" 
                          class="h-11 rounded-xl border-border/60 bg-white focus:ring-primary/10" 
                       />
                       <p v-if="form.errors.company_name" class="text-destructive text-xs">{{ form.errors.company_name }}</p>
                    </div>

                    <div class="space-y-2">
                       <Label for="industry" class="text-[13px] font-bold text-slate-700">Industry<span class="text-destructive ml-0.5">*</span></Label>
                       <Input 
                          id="industry" 
                    ...
                    <div class="space-y-2">
                       <Label for="company_size" class="text-[13px] font-bold text-slate-700">Company size<span class="text-destructive ml-0.5">*</span></Label>
                       <Select v-model="form.company_size">
                          ...
                          <SelectContent class="rounded-xl p-1">
                             <SelectItem value="1-10" class="rounded-lg">1-10 Employees</SelectItem>
                             <SelectItem value="11-50" class="rounded-lg">11-50 Employees</SelectItem>
                             <SelectItem value="51-200" class="rounded-lg">51-200 Employees</SelectItem>
                             <SelectItem value="201+" class="rounded-lg">201+ Employees</SelectItem>
                          </SelectContent>
                       </Select>
                    </div>
      ...
      <div class="bg-primary text-primary-foreground rounded-2xl p-6 md:p-8 flex flex-col md:flex-row items-center justify-between gap-6 shadow-xl shadow-primary/10">
         <div class="space-y-1 text-center md:text-left">
            <h3 class="text-lg font-display font-bold text-white">Butuh Arsitek Baru?</h3>
            <p class="text-sm text-primary-foreground/70">Mulai posting lowongan kerja dan dapatkan kandidat terbaik.</p>
         </div>
